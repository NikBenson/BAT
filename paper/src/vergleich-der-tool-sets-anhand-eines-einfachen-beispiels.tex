Die unterschiede der \ac{POSIX} Lösung, sowie der \ac{MPS} Lösung zum Compilerbau werden in diesem Kapitel strukturiert verglichen.
...
Wie in \autoref{fig:ard-player-mps-structure} zu er kennen ist, unterstützt die \ac{MPS} implementierung die volle {\ttfamily PlayerConfig}\footnote{In der Flutter--Implementierung, Version 6.4.0}.
...
\begin{figure}[H]
    ...
    \end{framed}
    \caption{Schritte des Compilierens der PlayerConfig Sprache mit \acs{MPS}}
...
Für größere Projekte wird es hier mit sicherheit Libraries geben, die das erstellen eines \ac{AST} erleichtern, allerdings ist dies ein Overhead, der hier nicht von Nöten ist.
...
Mit der Aufteilung der Generation auf die einzelnen Aspekte und dem daraus resultierenden \ac{SoC} ist der TextGen der \ac{JSON}--Sprache leicht verständlich.
